...
if __name__ == '__main__':
    ...
    products = KeyWi.select_product()  # 상품 DB에서 가져오기
    # products = open_json()  # 실패 목록에서 가져오기
    
    result=[]  # 성공 목록
    fail_products={}  # 실패 목록
    for products_id, *value in products:  # db에서 정보 가져와 크롤링
    # for products_id, value in products.items():  # 실패한 상품 목록에서 정보 가져와 크롤링
        ...
        try:
            ...
            try:
                ...
                print(e)
                print("상세 보기 실패")
                try: body.find_element(By.XPATH, "//strong[text()='이 상품은 현재 판매중지 된 상품입니다.']")
                except: print("로딩 실패")
