...
public class WebSocketConfig implements WebSocketMessageBrokerConfigurer {
    ...
    public void configureMessageBroker(MessageBrokerRegistry registry) {
        ...
        registry.enableSimpleBroker("/topic", "/queue")
                ...
                .setTaskScheduler(taskScheduler());            // 하트비트 스케줄러 설정
    ...
    /**
     * 커스텀 전송 핸들러 설정 (필요시 구현)
     */
    private Map<String, WebSocketTransportHandler> customTransportHandlers() {
        // 필요시 구현
    ...
    public void configureClientInboundChannel(ChannelRegistration registration) {
        registration.interceptors(stompHandler)
                .taskExecutor(clientInboundChannelExecutor());  // 전용 스레드 풀 설정
    }

    /**
     * 클라이언트 인바운드 채널을 위한 전용 스레드 풀
     */
    @Bean
    public ThreadPoolTaskExecutor clientInboundChannelExecutor() {
        ...
        executor.setThreadNamePrefix("ws-in-");   // 스레드 이름 접두사
        executor.initialize();
        return executor;
    }
    ...
    public ThreadPoolTaskExecutor clientOutboundChannelExecutor() {
        ...
        executor.setQueueCapacity(25);            // 작업 큐 크기
        executor.setThreadNamePrefix("ws-out-");  // 스레드 이름 접두사
        executor.initialize();
        return executor;
    }
}